...
#2 dire a gather les variables que l'on ne veut pas receuillir
...
wbp %>% 
  gather(-country, -indicator, key = year, value = values)
...
library(babynames)
df <- babynames
maryf <-filter(df, name == "Mary", sex == "F")
ggplot(df,aes(x = year, y = prop))+geom_line()
```
