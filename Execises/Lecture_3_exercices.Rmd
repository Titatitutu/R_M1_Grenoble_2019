...
```{r ex1}
fly <- filter(df, dep_time>1200 & dep_time<1400)
fly <- select(fly, origin, dest)
fly
```
...
```{r ex2}
isol <- filter(df, origin == "JFK" & dep_time>0 & dep_time <= 100| origin== "LGA" & dep_time>0 & dep_time <= 100)
isol <- select(isol, origin, dest)
ggplot(isol, aes(dest))+geom_bar()+
  coord_flip()
table(isol$origin, isol$dest)
```
...
df %>% 
  mutate(vitesse = distance / air_time,
         kmh= vitesse*60*1.609) %>% 
  select(kmh, vitesse, everything()) %>% 
...
```{r ex5}
df %>% 
  mutate(impact_retard = 100*arr_delay/air_time) %>% 
  select(impact_retard, arr_delay, air_time, everything())
...
```{r ex6}
df %>% 
  summarize(moyenne = mean(arr_delay, na.rm = TRUE),
            ecart_type = sd(arr_delay, na.rm = TRUE),
            min = min(arr_delay, na.rm = TRUE),
            max = max(arr_delay, na.rm = TRUE)) -> ex6
...
df %>% 
  group_by(carrier) %>% 
  summarize(moyenne = mean(arr_delay, na.rm= TRUE)) %>% 
  ggplot(aes(x = reorder(carrier, -moyenne), y = moyenne))+geom_col()
...
df %>%   
  ...
  mutate(vitesse = distance / air_time) %>%
  group_by(month) %>% 
  summarize(vitesse_mean = mean(vitesse, na.rm = TRUE)) %>% 
  ggplot(aes(x = month, y = vitesse_mean))+
...
df %>% 
  ...
  summarize(max_retard = max(dep_delay, na.rm = TRUE)) %>% 
  mutate(day_year = paste(day, month, sep = '.')) %>%
  group_by(origin) %>% 
  mutate(day_absolute = row_number()) %>% 
  ggplot(aes(x=day_absolute, y = max_retard, color = origin))+geom_point()
```
...
df %>% 
  ...
  ggplot(aes(x = dist_discrete, y = year))+geom_boxplot()
```

## Exercice 13

> combien de vols qui partent des trois aéroport de NY atterrsient dans des destinations au dessus de plus de 1000m s.n.m.?
